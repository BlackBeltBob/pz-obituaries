...
export function DeathForm({ slug, onClose, onSubmitted }: DeathFormProps) {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black/60 p-4">
      <div className="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-lg border border-slate-700 bg-slate-900 p-5">
        <h2 className="text-lg font-semibold text-slate-100">Record Death</h2>
        <form onSubmit={handleSubmit} className="mt-4 space-y-3">
          <div>
            <label className="text-sm text-slate-400">Cause of death</label>
            <textarea
              required
              value={causeOfDeath}
          ...
          <div>
            <span className="text-sm text-slate-400">Run length</span>
              ...
              {(Object.keys(emptyDuration) as Array<keyof RunDuration>).map((field) => (
                <div key={field}>
                  <input
                    ...
                    className="w-full rounded border border-slate-700 bg-slate-800 px-2 py-1 text-center text-slate-100"
                  />
          ...
          <div>
            <label className="text-sm text-slate-400">Favourite weapon</label>
            <input
              required
              type="text"
          ...
          <div>
            <label className="text-sm text-slate-400">Resting place screenshot</label>
            <input
              type="file"
              accept="image/*"
