...
export function SkillbooksPicker({ skillbooks, onChange }: SkillbooksPickerProps) {
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold text-slate-100">Skill Books</h2>
      <div className="mt-2 grid grid-cols-1 gap-1.5 sm:grid-cols-2">
        {catalog.map((name) => {
          ...
          return (
            ...
            >
              <span className={`text-sm ${tiers.some(Boolean) ? 'text-slate-200' : 'text-slate-500'}`}>{name}</span>
              <div className="flex shrink-0 divide-x divide-slate-700 overflow-hidden rounded border border-slate-700">
                {tiers.map((owned, index) => (
                  <button
                    ...
                    className={`h-6 w-6 text-xs transition ${
                      owned ? 'bg-amber-500 text-slate-900' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'
                    }`}
                  >
                    {index + 1}
                  </button>
