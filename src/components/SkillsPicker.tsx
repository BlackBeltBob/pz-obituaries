      ...
      <div className="mt-2 grid grid-cols-1 gap-1.5 sm:grid-cols-2">
        {catalog.map((name) => {
          ...
          return (
            ...
            >
              <span className={`text-sm ${level > 0 ? 'text-slate-200' : 'text-slate-500'}`}>{name}</span>
              <div className="flex shrink-0 items-center gap-2">
                <button
                  ...
                  className="flex h-5 w-5 items-center justify-center rounded border border-slate-700 text-xs text-slate-300 hover:border-slate-500 disabled:opacity-30"
                >
                  −
                </button>
                <span className="w-4 text-center text-sm text-slate-200">{level}</span>
                <button
                  type="button"
                  onClick={() => setLevel(name, level + 1)}
