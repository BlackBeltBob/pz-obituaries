          ...
          <li key={index} className="flex items-start gap-2">
            ...
            {editingIndex === index ? (
              <input
                ...
                className="flex-1 rounded border border-slate-600 bg-slate-800 px-1.5 py-0.5 text-sm text-slate-200"
              />
            ) : (
              <span
                onClick={() => startEditing(index)}
                className={`flex-1 cursor-text ${goal.achieved ? 'text-slate-300 line-through' : 'text-slate-300'}`}
                title="Click to edit"
              >
                {goal.description}
              </span>
